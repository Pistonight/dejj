...
use cu::pre::*;
use exstructs::{
    Goff, GoffMap, LType, Namespace, NamespaceMaps, NamespacedName, NamespacedTemplatedName,
    TemplateArg,
};
use tyyaml::{Prim, Tree};

use crate::CompileCommand;
...
impl NameParser {
    pub async fn parse(
        ...
        types: &GoffMap<LType>,
    ) -> cu::Result<GoffMap<NamespacedTemplatedName>> {
        let file = &compile_command.file;
        let command = cu::check!(
            TypeParseCommand::try_new(&self, &compile_command),
            "failed to create type parse command for {file}",
        )?;
        let original_cpp_file = Path::new(file);
        cu::ensure!(
            original_cpp_file.is_absolute(),
            "compilation unit name must be absolute: {file}",
        )?;
        cu::ensure!(
            original_cpp_file.exists(),
            "cannot find source file for compilation unit: {file}",
        )?;
...
"##,
        ...
        for (k, t) in types {
            ...
            let (name, namespace) = match t {
                LType::Typedef { name, .. } => (name, None),
                LType::EnumDecl(decl) | LType::UnionDecl(decl) | LType::StructDecl(decl) => {
                    (&decl.name_with_tpl, Some(&decl.enclosing))
                }
                _ => continue,
            };
        ...
        if !requests.is_empty() {
            let ast_nodes = match command.try_read_cached_ast(&source, &tokens) {
                ...
                None => cu::check!(
                    ...
                    "failed to invoke AST parse command for: {file}",
                )?,
            };
            command.parse_ast_nodes(&requests, &ast_nodes, &namespaces, &self, &mut final_names)?;
        }
...
impl TypeParseCommand {
    pub fn try_new(parser: &NameParser, command: &CompileCommand) -> cu::Result<Self> {
        // create a temporary file name based on the file path
        let hash = fxhash::hash64(&command.file);
        ...
        for include in &parser.system_header_paths {
    ...
    fn try_read_cached_ast(
        ...
        let old_output = match json::parse::<BTreeMap<String, Node<Ast>>>(&old_output) {
            Err(e) => {
                cu::error!(
                    "failed to parse cached output from {}: {e:?}",
                    self.out_file
                );
                return None;
            }
    ...
    async fn invoke(
        ...
        let tu_node = {
            ...
            if let Err(e) = child.co_wait_nz().await {
                ...
                cu::hint!(
                    "consider using extract.type-parser.abandon-typedefs config to exclude this name"
                );
                return Err(e);
            }
        ...
        if !tokens.is_empty() {
            cu::bail!(
                "not all tokens resolved from '{}': {tokens:?}",
                self.cpp_file
            );
        }
    ...
    fn parse_ast_nodes(
        ...
        for req in requests {
            ...
            match parse_ast(&node, req.namespace, ns, parser) {
                Ok(parsed) => {
                ...
                Err(e) => {
                    ...
                    cu::rethrow!(
                        e,
                        "failed to parse result node for type name: {}",
                        req.token
                    );
                }
            }
...
fn parse_ast(
    ...
    cu::ensure!(
        node.inner.len() == 1,
        "TypedefDecl node should have inner length 1"
    )?;
    ...
    cu::ensure!(
        node.inner.len() == 1,
        "ElaboratedType node should have inner length 1"
    )?;
    let node = &node.inner[0];
    ...
    let template_args = cu::check!(
        parse_template_spec_ast(node, nsmaps, parser),
...
fn parse_template_spec_ast(
    ...
    for n in &node.inner {
        ...
        cu::ensure!(
            n.inner.len() == 1,
            "TemplateArgument node should have inner length 1"
        )?;
        template_args.push(parse_template_arg_ast(&n.inner[0], ns, parser)?);
    }
...
fn parse_template_arg_ast(
    ...
    parse_template_arg_ast_recur(node, ns, parser, "", None)
}
    ...
    match &node.kind {
        ...
        Ast::BuiltinType { ty } => {
            let prim_name = match ty.qual_type.as_str() {
                ...
                "char" => parser.char_repr.to_str(),
                "wchar_t" => parser.wchar_repr.to_str(),
                _ => {
                    cu::bail!(
        ...
        Ast::TemplateSpecializationType { template_name } => {
            ...
            let template_args = cu::check!(
                parse_template_spec_ast(node, ns, parser),
            ...
            Ok(TemplateArg::Type(Tree::Base(
                NamespacedTemplatedName::with_templates(name, template_args),
            )))
        }
        Ast::RecordType { ty } => {
            cu::ensure!(
                node.inner.is_empty(),
                "RecordType node should have no inner nodes"
            )?;
            let name = cu::check!(
        ...
        Ast::EnumType { ty } => {
            cu::ensure!(
                node.inner.is_empty(),
                "EnumType node should have no inner nodes"
            )?;
            let name = cu::check!(
                to_namespaced_name_with_fallback(ns, &ty.qual_type, elaborated_qual_type),
        ...
        Ast::PointerType | Ast::LValueReferenceType | Ast::RValueReferenceType => {
            cu::ensure!(
                node.inner.len() == 1,
                "{:?} node should have inner length 1",
                node.kind
            )?;
            let node = &node.inner[0];
            let pointee = cu::check!(
                parse_template_arg_ast_recur(node, ns, parser, qualifier, None),
        ...
        Ast::QualType => {
            ...
            cu::ensure!(
                node.inner.len() == 1,
                "QualType node should have inner length 1"
            )?;
            let node = &node.inner[0];
            parse_template_arg_ast_recur(node, ns, parser, qualifier, None)
        }
        Ast::ParenType => {
            cu::ensure!(
                node.inner.len() == 1,
                "ParenType node should have inner length 1"
            )?;
            let node = &node.inner[0];
            parse_template_arg_ast_recur_paren_type(node, ns, parser)
        }
        ...
        Ast::MemberPointerType => {
            ...
            let thisty = cu::check!(
                parse_template_arg_ast_recur(&node.inner[0], ns, parser, "", None),
            ...
            let pointee = cu::check!(
                parse_template_arg_ast_recur(&node.inner[1], ns, parser, "", None),
...
fn parse_template_arg_ast_recur_paren_type(
    ...
    match &node.kind {
        Ast::FunctionProtoType => parse_template_arg_ast_recur(node, ns, parser, "", None),
        _ => {
        ...
        Err(e) => {
            ...
            match to_namespaced_name(ns, source) {
                ...
                Err(e2) => {
                    ...
                    cu::bail!(
                        "failed to convert source to namespaced name with fallback, please see logged errors"
                    );
                }
            }
        }
