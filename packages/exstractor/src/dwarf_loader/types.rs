...
use crate::dwarf::{self, Die, DieNode, Unit};
...
pub fn load_lstage(
    ...
    // add primitive types
    for p in Prim::iter() {
        types.insert(Goff::prim(p), LType::Prim(p));
    }
...
fn load_type_at<'a, 'b>(
    ...
    let ty = match entry.tag() {
        DW_TAG_unspecified_type => {
            ...
            match name {
                // std::nullptr_t
                "decltype(nullptr)" => LType::Prim(ctx.pointer_type),
                _ => {
        ...
        DW_TAG_typedef => {
            match cu::check!(
                ...
                Some(loff) => {
                    ...
                    if abandon {
                        LType::Alias(entry.to_global(loff))
                    } else {
                        LType::Typedef {
                            name: typedef_name,
                            target: entry.to_global(loff),
                        }
                    }
                }
            }
        ...
        DW_TAG_const_type | DW_TAG_volatile_type | DW_TAG_restrict_type => {
            match cu::check!(
                ...
                None => LType::Prim(Prim::Void),
                Some(loff) => LType::Alias(entry.to_global(loff)),
            }
        }
        // T[n]
        DW_TAG_array_type => {
            ...
            match array_len {
                ...
                None => make_ptr(goff),
                Some(len) => LType::Tree(Tree::Array(Box::new(Tree::Base(goff)), len)),
            }
        }
        // Subroutine
        DW_TAG_subroutine_type => {
            ...
            LType::Tree(Tree::Sub(subroutine_types))
        }
        // PTMD/PTMF
        DW_TAG_ptr_to_member_type => {
            ...
            if let Some(pointee_ty_loff) = pointee_ty_loff {
                ...
                if pointee_entry.tag() == DW_TAG_subroutine_type {
                    ...
                    LType::Tree(Tree::ptmf(this_ty_goff, subroutine_types))
                } else {
                    // PTMD
                    let pointee_ty_goff = entry.to_global(pointee_ty_loff);
                    LType::Tree(Tree::ptmd(this_ty_goff, Tree::Base(pointee_ty_goff)))
                }
            } else {
                // PTMD to void
                LType::Tree(Tree::ptmd(this_ty_goff, Tree::Base(Goff::prim(Prim::Void))))
            }
        }
        DW_TAG_base_type => LType::Prim(entry.prim_type()?),
...
fn load_enum_type_from_entry(entry: &Die<'_, '_>, ctx: &mut LoadTypeCtx) -> cu::Result<LType> {
    let offset = entry.goff();
...
fn load_union_type_from_entry(entry: &Die<'_, '_>, ctx: &mut LoadTypeCtx) -> cu::Result<LType> {
    let offset = entry.goff();
    let is_decl = cu::check!(
        entry.flag(DW_AT_declaration),
    ...
    if is_decl {
        ...
        return Ok(LType::UnionDecl(decl));
    }
    // remove templates from name for definition,
    // since we get those from DWARF nodes
    let name = cu::check!(
    ...
    if byte_size > u32::MAX as u64 {
        cu::bail!(
            "union at {offset} is too big (byte_size={byte_size}). This is unlikely to be correct"
        );
    }
    ...
    entry.for_each_child(|child| {
        ...
        match entry.tag() {
            DW_TAG_member => {
                ...
                let type_loff =
                    cu::check!(type_loff, "unexpected void-typed union member at {offset}")?;
...
fn load_struct_type_from_entry(entry: &Die<'_, '_>, ctx: &mut LoadTypeCtx) -> cu::Result<LType> {
    let offset = entry.goff();
    let is_decl = cu::check!(
        entry.flag(DW_AT_declaration),
    ...
    if is_decl {
        ...
        return Ok(LType::StructDecl(decl));
    }
    // remove templates from name for definition,
    // since we get those from DWARF nodes
    let name = cu::check!(
    ...
    if byte_size > u32::MAX as u64 {
        cu::bail!(
            "struct at {offset} is too big (byte_size={byte_size}). This is unlikely to be correct"
        );
    }
    let byte_size = byte_size as u32;
    ...
    cu::check!(
        result,
        "failed to process struct data for entry at {offset}"
    )?;
    ...
    if let Some(x) = conflicting_member_offset {
        cu::bail!(
            "found multiple members at the same offset 0x{x:x} for struct at {offset}: {members:#?}"
        );
    }
...
/// - DW_TAG_template_value_parameter
/// - DW_TAG_GNU_template_parameter_pack
fn load_template_parameter(
    entry: &Die<'_, '_>,
    out: &mut Vec<TemplateArg<Goff>>,
) -> cu::Result<()> {
    match entry.tag() {
        DW_TAG_template_type_parameter => {
            let type_loff = cu::check!(
...
fn load_data_symbol_at<'a, 'b>(
    node: DieNode<'a, 'b>,
    ctx: &mut LoadSymbolCtx,
) -> cu::Result<DieNode<'a, 'b>> {
    let entry = node.entry();
    let offset = entry.goff();
...
fn load_func_symbol_at<'a, 'b>(
    ...
    let linkage_name = cu::check!(
        super::load_func_linkage_name(&entry),
    ...
    if low_pc.is_none() {
        let is_inlined = cu::check!(
            super::load_func_is_inlined(&entry),
...
fn make_ptr(goff: Goff) -> LType {
    LType::Tree(Tree::ptr(Tree::Base(goff)))
}
