...
pub fn run(mut stage: HStage) -> cu::Result<Vec<HStage>> {
    ...
    for comp in &connected_components {
        ...
        for k in &comp.types {
            let t = cu::check!(
                ...
                "unexpected unconnected type goff {k} while splitting type map"
            )?;
            split_types.insert(*k, t);
        }
        let mut split_symbols = BTreeMap::new();
        for s in &comp.symbols {
            let (sym, info) = cu::check!(
                stage.symbols.remove_entry(s),
                "unexpected unconnected symbol {s} while splitting symbol map"
            )?;
    ...
    for split_stage in &mut split_stages {
        split_stage
            .types
            .extend(stage.types.iter().map(|(k, v)| (*k, v.clone())));
    }
